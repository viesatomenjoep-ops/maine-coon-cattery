...
export default function CustomerDetailPage() {
  ...
  const { customers, updateCustomer, deleteCustomer, kittens, updateKitten, litters, updateLitter, terms } = useStore();
  ...
  const handleAssignKitten = async () => {
    if(selectedKitten) {
      ...
      setSelectedKitten('');
      alert(`${cap(terms.young)} is succesvol gekoppeld en opgeslagen.`);
    }
  };

  const handleUnassignKitten = async (kittenId) => {
    if(confirm(`Weet je zeker dat je dit ${terms.young} wilt ontkoppelen?`)) {
  ...
  const handleAssignLitter = async () => {
    if(selectedLitter) {
      ...
      setSelectedLitter('');
      alert(`${cap(terms.litter)} is succesvol gekoppeld en opgeslagen.`);
    }
  };

  const handleUnassignLitter = async (litterId) => {
    if(confirm(`Weet je zeker dat je dit ${terms.litter} wilt ontkoppelen?`)) {
    ...
    <div className="pb-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <div className="space-y-4 text-sm text-forest-800">
            ...
            <div className="pt-4 border-t border-forest-900/10">
              <span className="block font-semibold text-forest-900 mb-2">Unieke klantenlink (persoonlijk portaal)</span>
        ...
        <div className="space-y-6">
          {/* Gekoppelde dieren */}
          <Card>
            <h2 className="mb-4 font-display text-xl text-forest-900">{`Gekoppelde ${terms.youngPlural}`}</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              <select value={selectedKitten} onChange={e=>setSelectedKitten(e.target.value)} className="flex-1 w-full rounded-xl border border-forest-900/10 p-2.5 text-sm min-w-0">
                <option value="">{`Kies een ${terms.young} om te koppelen…`}</option>
                {availableKittens.map(k => <option key={k.id} value={k.id}>{k.name}</option>)}
              </select>
              <Btn variant="brass" onClick={handleAssignKitten} className="shrink-0 w-full sm:w-auto justify-center">{`Koppel ${cap(terms.young)}`}</Btn>
            </div>

            <ul className="space-y-2.5">
              {assignedKittens.map(k => (
                <li key={k.id} className="flex items-center justify-between rounded-xl border border-forest-900/10 bg-white p-3">
                  <Link href={`/admin/cats/${k.id}`} className="font-medium text-forest-900 hover:text-brass-600">{k.name}</Link>
                  <button onClick={() => handleUnassignKitten(k.id)} className="text-xs text-red-600 hover:underline">Ontkoppelen</button>
                </li>
              ))}
              {assignedKittens.length === 0 && <p className="text-sm text-forest-600 italic">{`Geen ${terms.youngPlural} gekoppeld.`}</p>}
            </ul>
          </Card>

          {/* Gekoppelde nestjes */}
          <Card>
            <h2 className="mb-4 font-display text-xl text-forest-900">{`Gekoppelde ${terms.litterPlural} (wachtlijst/volgen)`}</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              <select value={selectedLitter} onChange={e=>setSelectedLitter(e.target.value)} className="flex-1 w-full rounded-xl border border-forest-900/10 p-2.5 text-sm min-w-0">
                <option value="">{`Kies een ${terms.litter} om te koppelen…`}</option>
                {availableLitters.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
              </select>
              <Btn variant="brass" onClick={handleAssignLitter} className="shrink-0 w-full sm:w-auto justify-center">{`Koppel ${cap(terms.litter)}`}</Btn>
            </div>

            <ul className="space-y-2.5">
              {assignedLitters.map(l => (
                <li key={l.id} className="flex items-center justify-between rounded-xl border border-forest-900/10 bg-white p-3">
                  <Link href={`/admin/litters/${l.id}`} className="font-medium text-forest-900 hover:text-brass-600">{l.name}</Link>
                  <button onClick={() => handleUnassignLitter(l.id)} className="text-xs text-red-600 hover:underline">Ontkoppelen</button>
                </li>
              ))}
              {assignedLitters.length === 0 && <p className="text-sm text-forest-600 italic">{`Geen ${terms.litterPlural} gekoppeld.`}</p>}
            </ul>
          </Card>
        </div>
      </div>
