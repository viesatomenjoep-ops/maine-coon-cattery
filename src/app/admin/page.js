        ...
        {tiles.map((t) => (
          <Link key={t.label} href={t.href} className="block group">
            <Card className="h-full flex flex-col items-center justify-center text-center p-4 sm:p-5 transition-all duration-300 hover:scale-[1.02] hover:border-brass-400 hover:shadow-xl hover:bg-white bg-cream-50/50 cursor-pointer">
              <span className="text-3xl sm:text-4xl mb-2 transition-transform group-hover:scale-110 duration-300 drop-shadow-sm">{t.icon}</span>
              <h2 className="font-display text-[1rem] sm:text-lg font-bold leading-tight text-forest-950 transition-colors group-hover:text-brass-600">{t.label}</h2>
              <p className="mt-1.5 text-[10px] sm:text-xs text-forest-600/80 font-medium">{t.desc}</p>
            </Card>
          </Link>
        ))}
      </div>
